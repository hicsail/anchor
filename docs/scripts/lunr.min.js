...
!function (){
  var t = function (e){
    var n = new t.Index(); return n.pipeline.add(t.trimmer,t.stopWordFilter,t.stemmer),e && e.call(n,n),n;
  }; t.version = '0.7.1',t.utils = {},t.utils.warn = function (t){
    return function (e){
      t.console && console.warn && console.warn(e);
    };
  }(this),t.utils.asString = function (t){
    return void 0 === t || t === null ? '' : t.toString();
  },t.EventEmitter = function (){
    this.events = {};
  },t.EventEmitter.prototype.addListener = function (){
    var t = Array.prototype.slice.call(arguments),e = t.pop(),n = t; if (typeof e !== 'function') {
    ...
    if (this.hasHandler(t)){
      var e = Array.prototype.slice.call(arguments,1); this.events[t].forEach((t) => {
        t.apply(void 0,e);
      });
    }
    ...
    return arguments.length && e != null && void 0 != e ? Array.isArray(e) ? e.map((e) => {
    ...
    var e = this.registeredFunctions[t]; if (!e) {
    ...
    var n = e.label && e.label in this.registeredFunctions; n || t.utils.warn('Function is not registered with pipeline. This may cause problems when serialising the index.\n',e);
  },t.Pipeline.load = function (e){
    var n = new t.Pipeline(); return e.forEach((e) => {
      var i = t.Pipeline.registeredFunctions[e]; if (!i) {
        throw new Error('Cannot load un-registered function: ' + e);
      } n.add(i);
    }),n;
  },t.Pipeline.prototype.add = function (){
    var e = Array.prototype.slice.call(arguments); e.forEach(function (e){
      t.Pipeline.warnIfFunctionNotRegistered(e),this._stack.push(e);
    },this);
  },t.Pipeline.prototype.after = function (e,n){
    t.Pipeline.warnIfFunctionNotRegistered(n); var i = this._stack.indexOf(e); if (i == -1) {
    ...
    var e = this._stack.indexOf(t); e != -1 && this._stack.splice(e,1);
    ...
    this._magnitude = void 0; var i = this.list; if (!i) {
    ...
    var e,t; for (t = 0; t < arguments.length; t++){
    ...
    var e = new t.SortedSet(); return e.elements = this.toArray(),e.length = e.elements.length,e;
  },t.SortedSet.prototype.union = function (t){
    var e,i,n; this.length >= t.length ? (e = this,n = t) : (e = t,n = this),i = e.clone(); for (var r = 0,o = n.toArray(); r < o.length; r++){
      i.add(o[r]);
    } return i;
  },t.SortedSet.prototype.toJSON = function (){
    return this.toArray();
  },t.Index = function (){
    this._fields = [],this._ref = 'id',this.pipeline = new t.Pipeline(),this.documentStore = new t.Store(),this.tokenStore = new t.TokenStore(),this.corpusTokens = new t.SortedSet(),this.eventEmitter = new t.EventEmitter(),this.tokenizerFn = t.tokenizer,this._idfCache = {},this.on('add','remove','update',() => {
      this._idfCache = {};
    });
    ...
    e.version !== t.version && t.utils.warn('version mismatch: current ' + t.version + ' importing ' + e.version); var n = new this(); return n._fields = e.fields,n._ref = e.ref,n.tokenizer = t.tokenizer.load(e.tokenizer),n.documentStore = t.Store.load(e.documentStore),n.tokenStore = t.TokenStore.load(e.tokenStore),n.corpusTokens = t.SortedSet.load(e.corpusTokens),n.pipeline = t.Pipeline.load(e.pipeline),n;
  },t.Index.prototype.field = function (t,e){
    var e = e || {},n = { name:t,boost:e.boost || 1 }; return this._fields.push(n),this;
  },t.Index.prototype.ref = function (t){
    return this._ref = t,this;
  },t.Index.prototype.tokenizer = function (e){
    var n = e.label && e.label in t.tokenizer.registeredFunctions; return n || t.utils.warn('Function is not a registered tokenizer. This may cause problems when serialising the index'),this.tokenizerFn = e,this;
  },t.Index.prototype.add = function (e,n){
    var i = {},r = new t.SortedSet(),o = e[this._ref],n = void 0 === n ? !0 : n; this._fields.forEach(function (t){
      var n = this.pipeline.run(this.tokenizerFn(e[t.name])); i[t.name] = n; for (var o = 0; o < n.length; o++){
        var s = n[o]; r.add(s),this.corpusTokens.add(s);
      }
    },this),this.documentStore.set(o,r); for (var s = 0; s < r.length; s++){
      for (var a = r.elements[s],h = 0,u = 0; u < this._fields.length; u++){
        var l = this._fields[u],c = i[l.name],f = c.length; if (f){
    ...
    var n = t[this._ref],e = void 0 === e ? !0 : e; if (this.documentStore.has(n)){
      var i = this.documentStore.get(n); this.documentStore.remove(n),i.forEach(function (t){
    ...
    var e = '@' + t; if (Object.prototype.hasOwnProperty.call(this._idfCache,e)) {
      return this._idfCache[e];
    } var n = this.tokenStore.count(t),i = 1; return n > 0 && (i = 1 + Math.log(this.documentStore.length / n)),this._idfCache[e] = i;
  },t.Index.prototype.search = function (e){
    var n = this.pipeline.run(this.tokenizerFn(e)),i = new t.Vector(),r = [],o = this._fields.reduce((t,e) => {
        return t + e.boost;
      ...
      var a = 1 / s.length * this._fields.length * o,h = this,u = this.tokenStore.expand(e).reduce((n,r) => {
        var o = h.corpusTokens.indexOf(r),s = h.idf(r),u = 1,l = new t.SortedSet(); if (r !== e){
          var c = Math.max(3,r.length - e.length); u = 1 / Math.log(c);
        }o > -1 && i.insert(o,a * s * u); for (var f = h.tokenStore.get(r),d = Object.keys(f),p = d.length,v = 0; p > v; v++){
          l.add(f[d[v]].ref);
        } return n.union(l);
      },new t.SortedSet()); r.push(u);
    },this); var a = r.reduce((t,e) => {
    ...
    for (var n = this.documentStore.get(e),i = n.length,r = new t.Vector(),o = 0; i > o; o++){
      var s = n.elements[o],a = this.tokenStore.get(s)[e].tf,h = this.idf(s); r.insert(this.corpusTokens.indexOf(s),a * h);
    } return r;
  },t.Index.prototype.toJSON = function (){
    return { version:t.version,fields:this._fields,ref:this._ref,tokenizer:this.tokenizerFn.label,documentStore:this.documentStore.toJSON(),tokenStore:this.tokenStore.toJSON(),corpusTokens:this.corpusTokens.toJSON(),pipeline:this.pipeline.toJSON() };
  },t.Index.prototype.use = function (t){
    var e = Array.prototype.slice.call(arguments,1); e.unshift(this),t.apply(this,e);
  },t.Store = function (){
    this.store = {},this.length = 0;
  },t.Store.load = function (e){
    var n = new this(); return n.length = e.length,n.store = Object.keys(e.store).reduce((n,i) => {
    ...
    var t = { ational:'ate',tional:'tion',enci:'ence',anci:'ance',izer:'ize',bli:'ble',alli:'al',entli:'ent',eli:'e',ousli:'ous',ization:'ize',ation:'ate',ator:'ate',alism:'al',iveness:'ive',fulness:'ful',ousness:'ous',aliti:'al',iviti:'ive',biliti:'ble',logi:'log' },e = { icate:'ic',ative:'',alize:'al',iciti:'ic',ical:'ic',ful:'',ness:'' },n = '[^aeiou]',i = '[aeiouy]',r = n + '[^aeiouy]*',o = i + '[aeiou]*',s = '^(' + r + ')?' + o + r,a = '^(' + r + ')?' + o + r + '(' + o + ')?$',h = '^(' + r + ')?' + o + r + o + r,u = '^(' + r + ')?' + i,l = new RegExp(s),c = new RegExp(h),f = new RegExp(a),d = new RegExp(u),p = /^(.+?)(ss|i)es$/,v = /^(.+?)([^s])s$/,g = /^(.+?)eed$/,m = /^(.+?)(ed|ing)$/,y = /.$/,S = /(at|bl|iz)$/,w = new RegExp('([^aeiouylsz])\\1$'),k = new RegExp('^' + r + i + '[^aeiouwxy]$'),x = /^(.+?[^aeiou])y$/,b = /^(.+?)(ational|tional|enci|anci|izer|bli|alli|entli|eli|ousli|ization|ation|ator|alism|iveness|fulness|ousness|aliti|iviti|biliti|logi)$/,E = /^(.+?)(icate|ative|alize|iciti|ical|ful|ness)$/,F = /^(.+?)(al|ance|ence|er|ic|able|ible|ant|ement|ment|ent|ou|ism|ate|iti|ous|ive|ize)$/,_ = /^(.+?)(s|t)(ion)$/,z = /^(.+?)e$/,O = /ll$/,P = new RegExp('^' + r + i + '[^aeiouwxy]$'),T = function (n){
      var a,h,i,o,r,s,u; if (n.length < 3) {
        return n;
      } if (o = n.substr(0,1),o == 'y' && (n = o.toUpperCase() + n.substr(1)),s = p,a = v,s.test(n) ? n = n.replace(s,'$1$2') : a.test(n) && (n = n.replace(a,'$1$2')),s = g,a = m,s.test(n)){
        var T = s.exec(n); s = l,s.test(T[1]) && (s = y,n = n.replace(s,''));
      }
      else if (a.test(n)){
        var T = a.exec(n); i = T[1],a = d,a.test(i) && (n = i,a = S,h = w,u = k,a.test(n) ? n += 'e' : h.test(n) ? (s = y,n = n.replace(s,'')) : u.test(n) && (n += 'e'));
      } if (s = x,s.test(n)){
        var T = s.exec(n); i = T[1],n = i + 'i';
      } if (s = b,s.test(n)){
        var T = s.exec(n); i = T[1],r = T[2],s = l,s.test(i) && (n = i + t[r]);
      } if (s = E,s.test(n)){
        var T = s.exec(n); i = T[1],r = T[2],s = l,s.test(i) && (n = i + e[r]);
      } if (s = F,a = _,s.test(n)){
        var T = s.exec(n); i = T[1],s = c,s.test(i) && (n = i);
      }
      else if (a.test(n)){
        var T = a.exec(n); i = T[1] + T[2],a = c,a.test(i) && (n = i);
      } if (s = z,s.test(n)){
        var T = s.exec(n); i = T[1],s = c,a = f,h = P,(s.test(i) || a.test(i) && !h.test(i)) && (n = i);
      } return s = O,a = c,s.test(n) && a.test(n) && (s = y,n = n.replace(s,'')),o == 'y' && (n = o.toLowerCase() + n.substr(1)),n;
    }; return T;
    ...
    var n = n || this.root,i = t.charAt(0),r = t.slice(1); return i in n || (n[i] = { docs:{} }),r.length === 0 ? (n[i].docs[e.ref] = e,void (this.length += 1)) : this.add(r,e,n[i]);
    ...
    var n = this.getNode(t),i = n.docs || {},e = e || []; return Object.keys(i).length && e.push(t),Object.keys(n).forEach(function (n){
      n !== 'docs' && e.concat(this.expand(t + n,e));
    },this),e;
